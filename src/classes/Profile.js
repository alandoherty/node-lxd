...
var Profile = utils.class_('Profile', {
  ...
  name: function(name, callback) {
    if (name === undefined) {
      ...
      this._client._request('POST /profiles/' + name, {name: name},
  ...
  delete: function(callback) {
    this._client._request('DELETE /profiles/' + this.name(), {},
      function(err, metadata) {
        callback(err);
      });
  },
  ...
  refresh: function(callback) {
    ...
    this._client._request('GET /profiles/' + this._metadata.name, {},
      function(err, metadata) {
        if (err) {
          callback(err);
        } else {
